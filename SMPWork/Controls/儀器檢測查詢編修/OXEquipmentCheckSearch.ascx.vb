Public Partial Class OXEquipmentCheckSearch
...
#End Region
    ...
    Protected Sub ldsSearchResult_Selecting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.LinqDataSourceSelectEventArgs) Handles ldsSearchResult.Selecting
        If Not IsUserAlreadyPutSearch Then
            ...
            Exit Sub
        End If
        Dim Result As IQueryable(Of CompanyLINQDB.標準樣本接收資料OX) = From A In DBDataContext.標準樣本接收資料OX Where A.日期時間 >= CType(Me.tbStartDate.Text, Date) And A.日期時間 < CType(Me.tbEndDate.Text, Date).AddDays(1) And (A.ShowSampleID = "" Or A.ShowSampleID Is Nothing) Select A
