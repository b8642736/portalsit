Public Class VoucherDetailEdit
...
#Region "查詢 方法:Search"
    ...
    Public Function Search(ByVal QryString As String) As List(Of CompanyORMDB.SQLServer.WELFARE.傳票明細)
        Dim ReturnValue As New List(Of CompanyORMDB.SQLServer.WELFARE.傳票明細)
        If String.IsNullOrEmpty(QryString) Then
            Return ReturnValue
        End If
        Return CompanyORMDB.SQLServer.WELFARE.傳票明細.CDBSelect(Of CompanyORMDB.SQLServer.WELFARE.傳票明細)(CompanyORMDB.SQLServerSQLQueryAdapter.ConnectionMoeEnum.ADO, QryString)
    End Function
#End Region
#Region "新增 方法:CDBInsert"
    <DataObjectMethod(DataObjectMethodType.Insert)> _
    Public Shared Function CDBInsert(ByVal SourceObject As CompanyORMDB.SQLServer.WELFARE.傳票明細) As Integer
...
#Region "刪除 方法:CDBDelete"
    ...
    Public Shared Function CDBDelete(ByVal SourceObject As CompanyORMDB.SQLServer.WELFARE.傳票明細) As Integer
        SourceObject.SQLQueryAdapterByThisObject = New CompanyORMDB.SQLServerSQLQueryAdapter(CompanyORMDB.SQLServerSQLQueryAdapter.ConnectionMoeEnum.ADO, CompanyORMDB.SQLServerSQLQueryAdapter.ConnectServerNameEnum.Server04m, "WELFARE")
...
#Region "修改 方法:CDBUpdate"
    ...
    Public Shared Function CDBUpdate(ByVal SourceObject As CompanyORMDB.SQLServer.WELFARE.傳票明細) As Integer
        SourceObject.SQLQueryAdapterByThisObject = New CompanyORMDB.SQLServerSQLQueryAdapter(CompanyORMDB.SQLServerSQLQueryAdapter.ConnectionMoeEnum.ADO, CompanyORMDB.SQLServerSQLQueryAdapter.ConnectServerNameEnum.Server04m, "WELFARE")
...
#Region "產生查詢字串至控制項 函式:MakQryStringToControl"
    ...
    Private Sub MakQryStringToControl()
        Dim SQLString As StringBuilder = New StringBuilder("Select A.* from 傳票明細 A  Where 1=1 ")

        If Not String.IsNullOrEmpty(tbVoucherNumber.Text) Then
            SQLString.Append(" and A.傳票編號 ='" & tbVoucherNumber.Text.Trim & "'")
        End If
        If Not String.IsNullOrEmpty(tbAccItem.Text) Then
            SQLString.Append(" and A.會計科目 ='" & tbAccItem.Text.Trim & "'")
        End If
