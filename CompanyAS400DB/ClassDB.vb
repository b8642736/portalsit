...
#Region "新增 方法:CDBInsert"
        ...
        Public Overridable Function CDBInsert() As Integer
            If CDBClassFieldNames.Count = 0 Then
                Return 0
            End If
            Dim DBService As New PublicClassLibrary.AS400SQLQuery
            ...
            Next
            Dim SQLString As String = "Insert into @#" & Me.CDBLibraryName & "." & Me.CDBFileName & "." & Me.CDBMemberName & " (" & FieldString & ") Values (" & ValueString & ")"
            ReturnValue += DBService.ExecuteNonQuery(SQLString)
            Return ReturnValue
...
#Region "更新 方法:CDBUpdate"
        ...
        Public Overridable Function CDBUpdate() As Integer
            ...
            Dim SQLString As String = "Update @#" & Me.CDBLibraryName & "." & Me.CDBFileName & "." & Me.CDBMemberName & " Set " & FieldValueString & " Where " & PKString
            ReturnValue += DBService.ExecuteNonQuery(SQLString)
            Return ReturnValue
        End Function
#End Region
#Region "刪除 方法:CDBDelete"
        ...
        Public Overridable Function CDBDelete() As Integer
            ...
            Dim SQLString As String = "Delete From @#" & Me.CDBLibraryName & "." & Me.CDBFileName & "." & Me.CDBMemberName & " Where " & PKString
            ReturnValue += DBService.ExecuteNonQuery(SQLString)
            Return ReturnValue
        End Function
#End Region
