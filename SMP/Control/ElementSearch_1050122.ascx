...
<asp:Button ID="btnSearchToExcel" runat="server" Text="查詢下載Excel"  Width="88px" /><br />
<asp:GridView ID="GridView1" runat="server" AllowPaging="True" AllowSorting="True"
    AutoGenerateColumns="False" DataSourceID="SDSAnalysisData" Width="2224px" PageSize="50" EnableModelValidation="True">
...
<asp:SqlDataSource ID="SDSAnalysisData" runat="server" ConnectionString="<%$ ConnectionStrings:SMP.My.MySettings.DefaultDBConnect %>"
    SelectCommand="SELECT 爐號,  鋼種, 材質, 站別, 序號, 判定, 日期, 時間, 操作員, DF, MD30, C, Si, Mn, P, S, Cr, Ni, Cu, Mo, Co, AL, Sn, Pb, Ti, Nb, V, W,O1 AS O2,N1 AS N2, H2, B, Ca, Mg, Fe, 分光儀編號 
                ,( SELECT 'Y'
...
FROM 分析資料  
WHERE 
            (日期 &gt;= @SetStartDate AND 日期 &lt;= @SetEndDate) AND 
            (爐號 = @SetStoveNumber OR @SetStoveNumber='ALL') AND 
            (鋼種 = @SetKind OR @SetKind='ALL') AND 
            (站別 Like @SetStationID 
                OR @SetStationID='ALL' 
                OR (@SetStationID='INGOT'  AND EXISTS (
                                        SELECT 1 
            	                        FROM [ForCustomerSteelAndType]
                                        WHERE [ForCustomerSteelAndType].[SteelKind]= [分析資料].[鋼種]
                                            AND [ForCustomerSteelAndType].[Type]= [分析資料].[材質]                  
            ...
            ORDER BY 日期 DESC,時間 DESC">
    <SelectParameters>
        <asp:Parameter DefaultValue="970101" Name="SetStartDate" />
        <asp:Parameter DefaultValue="971212" Name="SetEndDate" />
        <asp:Parameter Name="SetStoveNumber" DefaultValue="" />
        <asp:Parameter Name="SetKind" DefaultValue="" />
        <asp:ControlParameter ControlID="DDLStationName" Name="SetStationID" PropertyName="SelectedValue" />
    </SelectParameters>
</asp:SqlDataSource>
&nbsp; &nbsp;
