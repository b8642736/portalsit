...
#Region "連線OPCServer 方法:ConnectOPCServer"
    ...
    End Function
#End Region
#Region "中斷連線OPCServer 方法:DisConnectOPCServer"
    ''' <summary>
    ''' 中斷連線OPCServer
    ''' </summary>
    ''' <returns></returns>
    ...
    Public Function DisConnectOPCServer() As Boolean
        If daServerMgt.ServerState = Kepware.ClientAce.OpcDaClient.ServerState.CONNECTED Then
...
#Region "是否啟用訂閱 屬性:IsEnableSubscribe"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Property IsEnableSubscribe As Boolean = False
#End Region
#Region "加入訂閱資料項 函式:AddSubscriptionItem"
    ''' <summary>
    ''' 加入訂閱資料項
    ''' </summary>
    ...
    ''' <remarks></remarks>
    Public Sub AddSubscriptionItem(ByVal SetReadTagItemName As String, ByVal SetDataType As System.Type)
        Dim AddItems As List(Of OPCItemSubscribeDefine) = OPCItemSubscribeCollection.Values.ToList
        If Not IsNothing((From A In AddItems Where A.ReadTagItemName.Trim.ToUpper = SetReadTagItemName.Trim.ToUpper Select A).FirstOrDefault) Then
            Exit Sub
        End If
        Dim SetClientHandleID As Integer = 0
        For SetClientHandleIDTemp As Integer = 1 To Integer.MaxValue
            SetClientHandleID = SetClientHandleIDTemp
            If Not OPCItemSubscribeCollection.Keys.Contains(SetClientHandleID) Then
                Exit For
            End If
        Next

        Dim AddItem As Kepware.ClientAce.OpcDaClient.ItemIdentifier = New Kepware.ClientAce.OpcDaClient.ItemIdentifier '={New Kepware.ClientAce.OpcDaClient.ItemIdentifier("" ,activeServerSubscriptionHandle,SetClientHandleID ) '(From A In itemIdentifiers Where A.ItemName = "ns=2;s=APL1.CLX.Global.APL1_FUR_TEN" Select A).FirstOrDefault
        AddItem.ItemName = SetReadTagItemName.Trim
        AddItem.ClientHandle = SetClientHandleID
        AddItem.DataType = SetDataType
        OPCItemSubscribeCollection.Add(SetClientHandleID, New OPCItemSubscribeDefine(AddItem.ItemName, SetClientHandleID, AddItem.DataType))
        Dim OldValue() As Kepware.ClientAce.OpcDaClient.ItemIdentifier = Nothing
        If IsNothing(itemIdentifiers) Then
            ReDim itemIdentifiers(1)
        Else
            OldValue = itemIdentifiers.ToArray
            ReDim itemIdentifiers(itemIdentifiers.Count)
            For EachCount As Integer = 0 To OldValue.Count - 1
                itemIdentifiers(EachCount) = OldValue(EachCount)
            Next
        End If
        itemIdentifiers(itemIdentifiers.Count - 1) = AddItem
        If Me.IsEnableSubscribe = False Then
            Exit Sub
        End If
        daServerMgt.SubscriptionAddItems(activeServerSubscriptionHandle, {AddItem})
    End Sub

#End Region
#Region "移除訂閱資料項 函式:RemoveSubscriptionItem"
    ''' <summary>
    ''' 移除訂閱資料項
    ''' </summary>
    ''' <param name="ItemTagName"></param>
    ''' <remarks></remarks>
    Public Sub RemoveSubscriptionItem(ByVal ItemTagName As String)
        If IsNothing(itemIdentifiers) OrElse String.IsNullOrEmpty(ItemTagName) Then
            Exit Sub
        End If
        Dim RemoveItems() = (From A In itemIdentifiers Where (A.ItemName.ToUpper.Trim = ItemTagName.ToUpper.Trim Or ItemTagName = "_@ALL") Select A).ToArray
        If Not IsNothing(RemoveItems) AndAlso RemoveItems.Count > 0 Then
            If Me.daServerMgt.ServerState = Kepware.ClientAce.OpcDaClient.ServerState.CONNECTED Then
                daServerMgt.SubscriptionRemoveItems(activeServerSubscriptionHandle, RemoveItems)
            End If
            For Each EachItem In RemoveItems
                OPCItemSubscribeCollection.Remove(EachItem.ClientHandle)
            Next
        End If
    End Sub
#End Region
#Region "清除所有訂閱資料項 函式:ClearSubscriptionItems"
    ''' <summary>
    ''' 清除所有訂閱資料項
    ''' </summary>
    ''' <remarks></remarks>
    Public Sub ClearSubscriptionItems()
        RemoveSubscriptionItem("_@ALL")
    End Sub
#End Region
...
#Region "訂閱OPC資料 屬性:OPCItemSubscribt"
    ...
    Dim itemIdentifiers() As Kepware.ClientAce.OpcDaClient.ItemIdentifier
    ''' <summary>
    ''' 訂閱OPC資料
    ''' </summary>
    ...
    Public Sub OPCItemSubscribt(Optional maxAge As Integer = 0)
        ...
        ReDim itemIdentifiers(OPCItemSubscribeCollection.Count - 1)
        itemIndex = 0
        For Each EachItem As OPCItemSubscribeDefine In OPCItemSubscribeCollection.Values
            ...
            itemIdentifiers(itemIndex).ItemName = EachItem.ReadTagItemName.Trim   ' OPCItemNameTextBoxes(itemIndex).Text
            itemIdentifiers(itemIndex).ClientHandle = EachItem.ClientHandle ' itemIndex
            itemIdentifiers(itemIndex).DataType = EachItem.DataType    ' Assign types like this: Type.GetType("System.UInt16" for a word type)
            itemIndex += 1
        Next
        ...
        Try
            Dim SetIsEnableSubscribe As Boolean = True
            daServerMgt.Subscribe(clientSubscriptionHandle, SetIsEnableSubscribe, SubscriptionUpdateRate, revisedUpdateRate, DeadbandPecentage, itemIdentifiers, activeServerSubscriptionHandle)
            IsEnableSubscribe = SetIsEnableSubscribe
            ' Handle result:
...
#Region "取消訂閱 函式:Unsubscribe"
    ...
    Public Sub Unsubscribe()
        ...
        Try
            ...
            daServerMgt.SubscriptionRemoveItems(activeServerSubscriptionHandle, itemIdentifiers)
            daServerMgt.SubscriptionCancel(activeServerSubscriptionHandle)
            If Not daServerMgt.ServerState = Kepware.ClientAce.OpcDaClient.ServerState.CONNECTED Then
                daServerMgt.Disconnect()
            End If
        Catch ex As Exception
