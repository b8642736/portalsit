Namespace ACLIB
    ...
    Public Class ACJ050PF
        ...
        Sub New()
            MyBase.New("ACLIB", GetType(ACJ050PF).Name)
        End Sub
...
#Region "所有會計科目代號檔 屬性:AllACA010PFs"
        ...
        Shared ReadOnly Property AllACJ050PFs(ByVal GetTaiwanYear As Integer, Optional ByVal FactoryName As String = "SA") As List(Of ACJ050PF)
            Get
                Static LastGetDataTime As DateTime = Now
                Static _GetTaiwanYear As Integer = -1
                Static PreRunFactoryName As String = Nothing
                If Now.Subtract(LastGetDataTime).Seconds > 3 OrElse _GetTaiwanYear <> GetTaiwanYear OrElse IsNothing(PreRunFactoryName) OrElse PreRunFactoryName <> FactoryName Then
                    _AllACJ050PFs = Nothing
                End If
                If IsNothing(_AllACJ050PFs) Then
                    If FactoryName.ToUpper.Trim = "SA" Then
                        _AllACJ050PFs = ACJ050PF.CDBSelect(Of ACJ050PF)("Select * From @#ACLIB.ACJ050PF.SA Where ACCTYR='" & Format(GetTaiwanYear, "000") & "' Order by ACCTNO,DETLNO ", CompanyORMDB.AS400SQLQueryAdapter.ConnectionMoeEnum.ODBC)
                    Else
                        _AllACJ050PFs = ACJ050PF.CDBSelect(Of ACJ050PF)("Select * From @#ACAA.ACJ050PF." & FactoryName.ToUpper.Trim & " Where ACCTYR='" & Format(GetTaiwanYear, "000") & "' Order by ACCTNO,DETLNO ", CompanyORMDB.AS400SQLQueryAdapter.ConnectionMoeEnum.ODBC)
                    End If
                    PreRunFactoryName = FactoryName
                End If
