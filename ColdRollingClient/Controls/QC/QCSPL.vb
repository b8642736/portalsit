    ...
    Private Property EditCoilTreeNode() As CoilScanedTreeNode
        Get
            If IsNothing(_EditCoilTreeNode) Then
                If Not IsNothing(AboutCoilScanAndMachineProcess) AndAlso Not IsNothing(AboutCoilScanAndMachineProcess.CurrentEditCoilRunningItem) Then
                    _EditCoilTreeNode = AboutCoilScanAndMachineProcess.CurrentEditCoilRunningItem.CurrentRunningCoilScanedTreeNode
                Else
    ...
    ReadOnly Property TheStationCanUseBugDefines(Optional IsUpLineBug As Boolean = False) As List(Of QABUG)
        Get
            ...
            If Not IsNothing(_TempDatas) AndAlso _TempDatas.Count > 0 Then
                If IsUpLineBug Then
                    _TheStationCanUseBugDefines = (From A In _TempDatas Where A.SPLUseSortUpLineNumber > 0 Order By A.SPLUseSortUpLineNumber Select A).ToList
                Else
                    _TheStationCanUseBugDefines = (From A In _TempDatas Where A.SPLUseSortNumber > 0 Order By A.SPLUseSortNumber Select A).ToList
                End If
            Else
...
#End Region
    ...
    Private Sub AboutCoilScanAndMachineProcessEvent_CurrentEditCoilRunningItemChanged(OldValue As CoilRunningItem, CurrentValue As CoilRunningItem) Handles AboutCoilScanAndMachineProcessEvent.CurrentEditCoilRunningItemChanged
        ...
        tvSearchResult.Nodes.Clear()
        If Not IsNothing(AboutCoilScanAndMachineProcess.CurrentEditCoilRunningItem) AndAlso Not IsNothing(AboutCoilScanAndMachineProcess.CurrentEditCoilRunningItem.CurrentRunningCoilScanedTreeNode) AndAlso _
            Not IsNothing(AboutCoilScanAndMachineProcess.CurrentEditCoilRunningItem.CurrentRunningCoilScanedTreeNode.MyPPSSHAPFFlowAdapter) AndAlso Not IsNothing(AboutCoilScanAndMachineProcess.CurrentEditCoilRunningItem.CurrentRunningCoilScanedTreeNode.MyPPSSHAPFFlowAdapter.AboutRunProcessDatas) Then
            Dim AboutRunpProcessData As RunProcessData = (From A In AboutCoilScanAndMachineProcess.CurrentEditCoilRunningItem.CurrentRunningCoilScanedTreeNode.MyPPSSHAPFFlowAdapter.AboutRunProcessDatas Select A Order By A.SysCoilStartTime Descending).FirstOrDefault
            If Not IsNothing(AboutRunpProcessData) Then
                AboutCoilScanAndMachineProcess.CurrentEditCoilRunningItem.CurrentRunningCoilScanedTreeNode.Tag = AboutRunpProcessData
            End If
        End If
