Public Class LabRecordA2_Setting_Remark_DAT
...
#End Region
    ...
    Private W_Now主畫面_Enum As 主畫面_Enum = 主畫面_Enum.片語_基本檔
    Private Sub P_INIT()
        P_DropDownList_SetItem(Enum_DropDownList.片語基本檔_代號群組, queryREMARK_IDDropDownList)
        P_DropDownList_SetItem(Enum_DropDownList.檢驗項目基本檔_停用否, queryEFF_FLAGDropDown)
        queryEFF_FLAGDropDown.Items.Add("ALL：全部")

        P_DropDownList_SetItem(Enum_DropDownList.片語基本檔_條件檢查, queryCHECK_ASSAY_ITEMDropDown)
        queryCHECK_ASSAY_ITEMDropDown.Items.Add("ALL：全部")
        ModTools.SelectedIndexByText(queryCHECK_ASSAY_ITEMDropDown, "ALL")

        P_DropDownList_SetItem(Enum_DropDownList.檢驗項目基本檔_是否顯示, queryDISPLAY_FLAGDropDown)
        queryDISPLAY_FLAGDropDown.Items.Add("ALL：全部")
        ModTools.SelectedIndexByText(queryDISPLAY_FLAGDropDown, "ALL")
    ...
    Private Sub P_Detail_Display(ByVal fromRemark_ID As String)
        Me.hfRemark_ID.Value = fromRemark_ID
...
#Region "產生查詢字串至控制項  函式:MakeQryStringToControl"
    ...
    Private Sub MakeQryStringToControl(ByVal fromQueryMode As Enum_QueryMode)
        ...
        querySql.Append("FROM LabRecordA2_Remark_DAT " & vbNewLine)
        querySql.Append("WHERE 1=1 " & vbNewLine)

        Select Case fromQueryMode
            Case Enum_QueryMode.Master
                Dim W_Remark_KIND As String = W_ClsSystemNote.Fs_GetStrBefor(queryREMARK_IDDropDownList.SelectedValue, W_ClsSystemNote.Display_Symbol)

                If queryREMARK_IDTextBox.Text = "" Then
                    querySql.Append("AND REMARK_ID LIKE '" & W_Remark_KIND & "%' " & vbNewLine)
                Else
                    querySql.Append("AND REMARK_ID ='" & String.Concat(W_Remark_KIND, queryREMARK_IDTextBox.Text) & "' " & vbNewLine)
                End If

                If queryCONTEXTTextBox.Text <> "" Then
                    querySql.Append("AND CONTEXT LIKE '%" & queryCONTEXTTextBox.Text & "%' " & vbNewLine)
                End If
                If queryDISPLAY_SEQTextBox.Text <> "" Then
                    querySql.Append("AND DISPLAY_SEQ =" & queryDISPLAY_SEQTextBox.Text & " " & vbNewLine)
                End If

                Dim W_DISPLAY_FLAG As String = W_ClsSystemNote.Fs_GetStrBefor(queryDISPLAY_FLAGDropDown.SelectedValue, W_ClsSystemNote.Display_Symbol)
                If W_DISPLAY_FLAG <> "ALL" Then
                    querySql.Append("AND DISPLAY_FLAG ='" & W_DISPLAY_FLAG & "' " & vbNewLine)
                End If
                ...
                Dim W_CHECK_ASSAY_ITEM As String = W_ClsSystemNote.Fs_GetStrBefor(queryCHECK_ASSAY_ITEMDropDown.SelectedValue, W_ClsSystemNote.Display_Symbol)
                If W_CHECK_ASSAY_ITEM <> "ALL" Then
                    querySql.Append("AND CHECK_ASSAY_ITEM ='" & W_CHECK_ASSAY_ITEM & "' " & vbNewLine)
                End If

                querySql.Append("ORDER BY DISPLAY_SEQ , LEN(CONTEXT) ,CONTEXT " & vbNewLine)
                Me.hfSQL_Master.Value = queryTime & "|" & querySql.ToString

            Case Enum_QueryMode.Detail_Edit
                querySql.Append("AND REMARK_ID ='" & Me.hfRemark_ID.Value & "' " & vbNewLine)
...
#Region "CRUD"
    ...
    Protected Function Search(ByVal fromSQL As String) As List(Of SQLServer.QCDB01.LabRecordA2_Remark_DAT)
        If String.IsNullOrEmpty(fromSQL) OrElse fromSQL = "" _
            OrElse fromSQL.Split("|").Count < 2 Then
            Return New List(Of SQLServer.QCDB01.LabRecordA2_Remark_DAT)
        End If

        Return SQLServer.QCDB01.LabRecordA2_Remark_DAT.CDBSelect _
                            (Of SQLServer.QCDB01.LabRecordA2_Remark_DAT) _
    ...
    Public Function Search_Master(ByVal fromSQL As String) As List(Of SQLServer.QCDB01.LabRecordA2_Remark_DAT)
        Return Search(fromSQL)
    End Function

    <DataObjectMethod(DataObjectMethodType.Select)>
    Public Function Search_Detail(ByVal fromSQL As String) As List(Of SQLServer.QCDB01.LabRecordA2_Remark_DAT)
        Return Search(fromSQL)
    End Function

    <DataObjectMethod(DataObjectMethodType.Insert)>
    Public Function Insert(ByVal fromObj As SQLServer.QCDB01.LabRecordA2_Remark_DAT) As Integer
        Return fromObj.CDBInsert()
    End Function

    <DataObjectMethod(DataObjectMethodType.Update)>
    Public Function Update(ByVal fromObj As SQLServer.QCDB01.LabRecordA2_Remark_DAT) As Integer
...
#Region "ListView1"
    ...
    Private Sub ListView1_ItemInserted(sender As Object, e As ListViewInsertedEventArgs) Handles ListView1.ItemInserted
        ...
        GridView1.SelectedIndex = -1
        P_Detail_Display(Me.hfRemark_ID.Value)
    End Sub
    ...
    Private Sub ListView1_ItemInserting(sender As Object, e As ListViewInsertEventArgs) Handles ListView1.ItemInserting
        ...
        W_KeyArray = New String() {"REMARK_ID", "DISPLAY_FLAG", "CHECK_ASSAY_ITEM", "EFF_FLAG"}
        ...
        For Each eachItemKey As String In W_KeyArray
            ...
            W_Value = W_ClsSystemNote.Fs_GetStrBefor(W_Obj_DropDownList.SelectedValue, W_ClsSystemNote.Display_Symbol)
        ...
        'REMARK_ID
        '---------------------------------------------------------------
        W_Value = Fs_GetNewRemark_ID_Number(e.Values("REMARK_ID"))
        e.Values("REMARK_ID") = W_Value
        hfRemark_ID.Value = W_Value
        '---------------------------------------------------------------
    ...
    Private Sub ListView1_ItemUpdating(sender As Object, e As ListViewUpdateEventArgs) Handles ListView1.ItemUpdating
        ...
        W_KeyArray = New String() {"DISPLAY_FLAG", "CHECK_ASSAY_ITEM", "EFF_FLAG"}
    ...
    Private Function Fs_GetNewRemark_ID_Number(ByVal fromRemark_ID_Kind As String) As String
        ...
        Dim W_Remark_ID_NumLeng As Integer = 2
        Dim W_Remark_ID_NumNow As String
        Dim W_Remark_ID_NumNew As String = Space(W_Remark_ID_NumLeng).Replace(Space(1), "0")

        W_SQL.Append("DECLARE @W_Remark_ID_KIND NVARCHAR(10) " & vbNewLine)
        W_SQL.Append("DECLARE @_W_Remark_ID_Num   NVARCHAR(10) " & vbNewLine)
        W_SQL.Append("SET @W_Remark_ID_KIND = '" & fromRemark_ID_Kind & "' " & vbNewLine)
        W_SQL.Append("SET @_W_Remark_ID_Num = '" & W_Remark_ID_NumNew & "' " & vbNewLine)
        W_SQL.Append(" " & vbNewLine)
        W_SQL.Append(" " & vbNewLine)
        W_SQL.Append("SELECT  ISNULL(  " & vbNewLine)
        W_SQL.Append("					 MAX(Remark_ID)  " & vbNewLine)
        W_SQL.Append("					,CONCAT(@W_Remark_ID_KIND, @_W_Remark_ID_Num ) " & vbNewLine)
        W_SQL.Append("				) Remark_ID_max " & vbNewLine)
        W_SQL.Append(" " & vbNewLine)
        W_SQL.Append("FROM [LabRecordA2_Remark_DAT] " & vbNewLine)
        W_SQL.Append("WHERE Remark_ID LIKE  CONCAT(@W_Remark_ID_KIND ,  '%' ) " & vbNewLine)

        W_Remark_ID_NumNow = W_SQLQueryAdapter.SelectQuery(W_SQL.ToString).Rows(0).Item(0)

        W_Remark_ID_NumNew = W_Remark_ID_NumNow.Substring(fromRemark_ID_Kind.Length)
        W_Remark_ID_NumNew = Hex(Convert.ToInt32(W_Remark_ID_NumNew, 16) + 1).PadLeft(W_Remark_ID_NumLeng, "0")

        Return String.Format("{0}{1}", fromRemark_ID_Kind, W_Remark_ID_NumNew)
    End Function
    ...
    Private Sub ListView1_PreRender(sender As Object, e As EventArgs) Handles ListView1.PreRender
        ...
        Dim W_RemarkIDNumber As String
        ...
        W_DDList1.Add(New ListView1_DropDownList("REMARK_ID", Enum_DropDownList.片語基本檔_代號群組))
        W_DDList1.Add(New ListView1_DropDownList("DISPLAY_FLAG", Enum_DropDownList.檢驗項目基本檔_是否顯示))
        W_DDList1.Add(New ListView1_DropDownList("CHECK_ASSAY_ITEM", Enum_DropDownList.片語基本檔_條件檢查))
        ...
        For Each eachItem1 As ListView1_DropDownList In W_DDList1
            ...
            If W_DoModeIs = EditMode_Enum.編輯 Then
                ...
                If eachItem1.Note_Type = Enum_DropDownList.片語基本檔_代號群組 Then
                    W_RemarkIDNumber = W_SelectValue.Substring(2)
                    ...
                    W_Obj_TextBox.Text = W_RemarkIDNumber
                End If
            ...
            Else
                ...
                '---------------------------------------------------------------------------------------
                If eachItem1.Note_Type = Enum_DropDownList.片語基本檔_代號群組 Then
                    W_SelectValue = W_ClsSystemNote.Fs_GetStrBefor(queryREMARK_IDDropDownList.SelectedValue, W_ClsSystemNote.Display_Symbol)
                    ModTools.SelectedIndexByValue(W_Obj_DropDownList, W_SelectValue)
                End If
        ...
        '---------------------------------------------------------------------------------------
        'Remark_ID
        W_Obj_TextBox = CType(W_Obj_ListViewItem.FindControl(String.Concat("Remark_ID", C_Fix_TextBox)), TextBox)
        W_Obj_DropDownList = CType(W_Obj_ListViewItem.FindControl(String.Concat("Remark_ID", C_Fix_DropDown)), DropDownList)
